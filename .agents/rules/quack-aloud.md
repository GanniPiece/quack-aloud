...
When the user invokes `/duck` or asks to file thoughts on the canvas, read and follow `docs/duck-guide.md`. It holds the shared organising rules, guidance modes, MCP workflow, local-file fallback, and canvas schema. `.agents/workflows/duck.md` is the explicit workflow entry point.

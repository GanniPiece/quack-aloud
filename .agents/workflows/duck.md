...
1. Pick the project. If the text asks for a new project ("new project", "新專案", "開新專案", "新的…專案"), create the folder `data/projects/p-YYYYMMDD-HHMMSS/` (current local time) with `project.json` = `{"name":"<name>","createdAt":<ms epoch>}` and a first canvas `c-YYYYMMDD-HHMMSS.json` = `{"version":1,"name":"Main","groups":[],"nodes":[],"edges":[],"messages":[]}`. If it asks for a new canvas ("new canvas", "新畫布", "開新畫布"), add such a `c-….json` (with the requested name) to the open project and use it; tell the user to select it in the picker. Otherwise run `curl -s http://localhost:8787/api/projects` (projects and their canvases, sorted by last update), take the first project and its first canvas, and edit `data/projects/<pid>/<cid>.json`. If the server is not running, tell the user to run `npm run dev` and stop. If there is more than one project and the intent is unclear, ask which one.

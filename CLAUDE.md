...
Claude Code is an external agent, independent of the browser's Claude/OpenAI API provider. Browser chat gets its instructions from `server/prompt.ts`, not this file. Codex has `AGENTS.md` and a `$duck` skill; Antigravity has `.agents/rules/` and `.agents/workflows/`. All three use the same canvases and organising rules.

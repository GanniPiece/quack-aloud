---
name: duck
description: Be the rubber duck of Quack Aloud. Use when the user types /duck, thinks out loud, or asks to file thoughts as cards on the canvas; reads what they said, files it in the open project, replies in one breath.
---
# Duck

The text after `/duck` (or the message that invoked this skill) is what the user is thinking out loud. If it contains "guide", "引導", or "質疑", guidance is on for this turn; otherwise organise only. Follow the rules in `.agents/rules/quack-aloud.md` throughout.

If the `quack-aloud` MCP server is configured (Antigravity: `agy mcp add` or Customizations › MCP, either the HTTP server `http://localhost:8787/mcp` with the bearer token from Settings › MCP access, or the stdio command `npx tsx server/mcp.ts` with absolute paths; see README › MCP server), use its tools instead of the file steps below: `list_projects` → `read_canvas` → `duck_turn` (or `create_project` / `create_canvas` first). Reply with the same text you passed as "reply", then stop.

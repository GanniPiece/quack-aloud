...
1. Find the open project with `curl -s http://localhost:8787/api/projects` (first entry is the most recently used). If the server is not running, tell the user to run `npm run dev` and stop. If several projects exist and it is unclear which one, ask.

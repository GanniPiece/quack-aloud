...
1. Pick the project. If the thought asks for a new project ("new project", "新專案", "開新專案", "新的…專案") create `data/projects/<slug>.json` with `{"version":1,"name":"<name>","groups":[],"nodes":[],"edges":[],"messages":[]}` (slug: lowercase letters, digits, dashes) and use it; tell the user to select it in the picker. Otherwise find the open one with `curl -s http://localhost:8787/api/projects` (first entry is the most recently used). If the server is not running, tell the user to run `npm run dev` and stop. If several projects exist and it is unclear which one, ask.
